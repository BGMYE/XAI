...
try:
    with sync_playwright() as p:
        ...
        try:
            ...
            page.locator(".xai-quick-grid button").filter(has_text="新建画布").click()
            expect(page.locator(".xai-shell-content.is-editor")).to_be_visible()
            ...
            page.locator(".xai-shell-mode").get_by_role("button", name="简洁模式", exact=True).click()
            prompt = page.get_by_role("textbox", name="创作提示词", exact=True)
            prompt.fill("STUDIO_V2_PERSISTENCE_SMOKE")
            page.wait_for_timeout(1200)
            assert page.locator(".xai-storage-error").count() == 0, "Canvas autosave failed"
